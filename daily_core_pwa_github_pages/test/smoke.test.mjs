...
test("service worker: v61, purge old daily-core caches, network-first for documents", () => {
  const sw = fs.readFileSync(path.join(root, "service-worker.js"), "utf8");
  assert.match(sw, /const CACHE_NAME\s*=\s*"daily-core-v61"/);
  assert.match(sw, /PERSONA_QS\s*=\s*"\?v=61"/);
